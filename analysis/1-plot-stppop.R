...
## import data
df_input <- read_csv(
    here::here("outputs", "cohorts", "input_1_stppop.csv"),
    col_types = cols(
...
plot_stppop_bar <- df_stppop %>%
    ...
    ggplot() + geom_col(aes(x=registered/1000000, y =name, fill=registered), colour = 'black') +
               ...
               labs(
                ...
                theme(panel.grid.major.y = element_blank(),
                      panel.grid.minor.y = elemenet_blank(),
...
## Save plot
dir.create(here::here("output", "plots")) #  Put the output in a subfolder, it needs to be created

ggsave(
    plot = plot_stppop_bar,
    filename = "plots_stppop_bar.png", path = here::here("outputs", "plots"),
    units = "cm",
    height = 15,
    width = 15
)
